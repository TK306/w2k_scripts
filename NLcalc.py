...
def mapall():  # calculate all BZ coarsely
	...
	for kz in range(nz + 1):
		for ky in range(ny + 1):
			...
			kb.main(w2k.filepath('.klist_band'), kmeshx=nx + 1,
					kpath=[[0, ky / ny, kz / nz], [1, ky / ny, kz / nz]],
					index_ls=[])
			sp.call(['cp', w2k.filepath('.klist_band'), outfol + 'klist/' + name + '.klist_band'])
...
def make_klist_NL(ba_ls):  # make .klist_band files
	...
	def int2vec(val):
		vec = [np.mod(val, 10000), np.floor(np.mod(val, 10000 ** 2) / 10000), np.floor(val / 10000 ** 2)]
	...
	for ba in ba_ls:
			...
			for dkz in dkz_ls:
				for dky in dky_ls:
					for dkx in dkx_ls:
						...
						prog += 1
						nn = np.floor(
							prog / (len(nl) * len(dkx_ls) * len(dky_ls) * len(dkz_ls)) * 100 * pdens * 100) / (
									 pdens * 100)
						nsec = dt.datetime.now().second
						if np.floor(nn * pdens) / pdens > cp or ((float(nsec) % 20) == 0 and nsec != nt):
							nt = dt.datetime.now().second
							cp = np.floor(
								prog / (len(nl) * len(dkx_ls) * len(dky_ls) * len(dkz_ls)) * 100 * pdens) / pdens
		...
		for kp in klist_all:
			if (len(klist_out) > km):
				...
				kb.sonomama(kbout + 'klist_' + str(fn) + '.klist_band', klist_out, 5000, 0)
				klist_out = []
				fn += 1
...
def calc_NL_from_klists(ba_ls):  # calculate band dispersion
	...
	for ba in ba_ls:
		...
		for kbf in fl:
			...
			print('IF YOU WANT TO STOP THIS SCRIPT, MAKE stop.txt FILE IN ' + w2k.case_path)
			print('')
